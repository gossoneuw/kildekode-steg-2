  ...
  else
  {
	...
	if (preg_match_all('[\/|\$|"|\/|<|>|\(|\)]', $username)) {
                        $error = array("Illegal characters used during name with forereg api. name used: ", $username);
  ...
  if($stmt->execute()){
    $stmtquery2 = "INSERT INTO foreleserv2(foreleserid, idemne, godkjent) VALUES (?,?,?)";
    if (!$stmt2 = $mysqli->prepare($stmtquery2)) {
	die("sql error: hei2");
	}

    $stmt2->bind_param("sss", $last_id, $emneid, $godkjent);
