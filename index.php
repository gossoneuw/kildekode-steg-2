...
<!DOCTYPE html>
 ...
 <body>
        ...
        <h1>Home</h1>
	<form action="http://158.39.188.203/Dokumentasjon/dokumentasjon.html">
    	<input type="submit" value="Dokumentasjon" />
	</form>
		...
		<form method="post">
            		...
            		<div>
                        ...
                        <?php if (!$emneid == 0): ?>
				<h2>"Forelesere" i dette emne.</h2>
			  	<?php 
				$bq = "SELECT bildeid FROM foreleserv2 WHERE idemne='$emneid' AND bildeid !=''";
				$bq_result = $mysqli->query($bq);
				while($imgs = $bq_result->fetch_assoc())  {?>
					<img src="brukerbilde/<?php echo $imgs['bildeid'] ?>" width='200' height='200'>
				<?php } ?>
                                <textarea readonly id="messagesBox" rows="15" cols="65" style="text-align:right";>
                                        <?php 
                                                ...
                                                while($msgs = $q_result->fetch_assoc()) { 
                                                        ...
                                                        $result = $mysqli->query("SELECT * FROM meldingv2 WHERE til = '$idmelding' and msgtype = 3");
